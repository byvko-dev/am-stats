...
func HandleStatsImageExport(c *fiber.Ctx) error {
	...
	export, err := stats.ExportSessionAsStruct(request.PlayerID, request.TankID, request.Realm, request.Days, 0, "")
	if err != nil {
		log.Println(err)
		return c.Status(http.StatusInternalServerError).JSON(fiber.Map{
			"error": err.Error(),
...
func HandleStatsJSONExport(c *fiber.Ctx) error {
	...
	export, err := stats.ExportSessionAsStruct(request.PlayerID, request.TankID, request.Realm, request.Days, request.TankLimit, request.Sort)
	if err != nil {
		log.Println(err)
		return c.Status(http.StatusInternalServerError).JSON(fiber.Map{
			"error": err.Error(),
