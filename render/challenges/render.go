...
func getPlayerChall(pid int) (blocks []challengeBlock, err error) {
	...
	block.width = (render.BaseCardWidth - (int(render.TextMargin) * 8)) / 3
	...
	block.textSize = render.FontSize
...
func addChallengeCtx(block challengeBlock) (challengeBlock, error) {
	...
	if block.isColored == true {
		...
		ctx.DrawRoundedRectangle(0, 0, float64(block.width), float64(block.height), render.FontSize)
		ctx.Fill()
	}
	if block.textCoeff == 0 {
		block.textCoeff = 0.6
	}
	topRow := block.textSize + render.TextMargin
	...
	if block.isLocked {
		...
		if err := ctx.LoadFontFace(render.FontPath, (block.textSize * 0.75)); err != nil {
			return block, err
		}
		t2X := render.TextMargin
		t2Y := topRow + render.TextMargin
		ctx.DrawStringWrapped(t2, t2X, t2Y, 0.0, 0.0, (float64(block.width) - render.TextMargin*2), 1.5, gg.AlignLeft)
		...
		if block.prizeTxt != "" {
			...
			pX := (float64(block.width) - render.TextMargin*2 - pW) / 2
			pY := float64(block.height) - render.TextMargin
	...
	if block.position > 0 {
		...
		lbY := float64(block.height) - render.TextMargin
	...
	saY := topRow + render.TextMargin + saH
	...
	if err := ctx.LoadFontFace(render.FontPath, (block.textSize * scoreCoeff)); err != nil {
		return block, err
	}
	sW, _ := ctx.MeasureString(score)
	sX := (float64(scoreWidth) - sW) / 2
	sY := saY + (float64(block.height)-render.TextMargin-saY)/2 // Center betweeen position and score alt text
	...
	ctx.SetColor(block.longTxtColor)
	ltX := scoreWidth
	ltY := topRow + render.TextMargin
	ctx.DrawStringWrapped(block.longText, ltX, ltY, 0.0, 0.0, (rightWidth - render.TextMargin), 1.5, gg.AlignLeft)
